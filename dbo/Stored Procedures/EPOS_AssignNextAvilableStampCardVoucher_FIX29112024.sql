...
-- =============================================
CREATE PROCEDURE [dbo].[EPOS_AssignNextAvilableStampCardVoucher_FIX29112024] ( 
												@ClientId INT,
												@ProfileTypeId INT,
												@Quantity INT,
												@MemberId INT,
...
--PRINT '------------------------'
	 ...
	 --SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	  ...
	  DECLARE @Result NVARCHAR(500) ,	@ResultQty INT = 0,@VoucherProfile NVARCHAR(250) = '',@StartDate DATETIME

	  SELECT @StartDate = CreateDate FROM TrxHeader Where TrxId = @TrxId
