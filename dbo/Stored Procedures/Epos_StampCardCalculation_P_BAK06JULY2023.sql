...
-- =============================================
CREATE PROCEDURE [dbo].[Epos_StampCardCalculation_P_BAK06JULY2023]
	...
	@PromotionStampCardCalculation NVARCHAR(MAX) = null
	)
AS
BEGIN
	...
	DECLARE @BeforeValue decimal(18,2) = 0,@AfterValue decimal(18,2) = 0,@VoucherProfileId INT
	...
	------------------------------------------------------------
	DECLARE @ExistingQuantity INT = 0,@StampCount INT = 0
	...
	IF ISNULL (@UserId,0) > 0
	BEGIN
		UPDATE PromotionStampCounter SET BeforeValue = 0,OnTheFlyQuantity = 0 WHERE UserId =  @UserId AND PromotionId = @PromotionId
	END
	ELSE IF ISNULL (@DeviceIdentifier,0) > 0
	BEGIN 
		UPDATE PromotionStampCounter SET BeforeValue = 0,OnTheFlyQuantity = 0 WHERE DeviceIdentifier =  @DeviceIdentifier AND PromotionId = @PromotionId
	END

	delete from [VirtualStampCard] 
	where trxid=@TrxId AND @TrxId > 0 AND PromotionId = @PromotionId

	DECLARE @Result TABLE(Result INT,  RewardId INT,  RewardName NVARCHAR(250),VoucherValue DECIMAL(18,2),
	DefaultVoucher NVARCHAR(25),ExistingQuantity decimal(18,2),VoucherSubType NVARCHAR(25),StampCardMultiplier float)
	...
	BEGIN
		...
		--select * from [VirtualStampCard] where trxid=@TrxId and Linenumber=@LineNumber AND @TrxId > 0
		--,@PromotionThreshold = QualifyingProductQuantity
		SELECT @QualifyingProductQuantity = QualifyingProductQuantity,@PromotionCategoryId = PromotionCategoryId,@PromotionOfferTypeId =PromotionOfferTypeId , @Reward = ISNULL(Reward,''),@VoucherProfileId = ISNULL(VoucherProfileId,0),@StampCardMultiplier = ISNULL(StampCardMultiplier,1)
		...
		BEGIN
			...
			select TOP 1 @VoucherValue = VD.OfferValue ,@VoucherSubType = VT.Name from VoucherDeviceProfileTemplate VD  INNER JOIN VoucherSubType VT   ON VD.VoucherSubTypeId = VT.VoucherSubTypeId Where VD.id = @VoucherProfileId
		...
		SELECT * INTO #LineItem FROM [dbo].[fnSplitString] (@PromotionStampCardCalculation,'&')
		--SELECT * FROM #LineItem

DECLARE @splitdata NVARCHAR(MAX)
DECLARE LineItemCursor CURSOR LOCAL FORWARD_ONLY READ_ONLY FOR               
SELECT splitdata FROM #LineItem                             
OPEN LineItemCursor                                                  
	...
	BEGIN 
		...
		SET @splitdata = '{"L":"'+@splitdata+'"}'
		SET @splitdata = REPLACE(@splitdata, 'V', '","V":"')
		SET @splitdata = REPLACE(@splitdata, 'Q', '","Q":"')

		SET @LineNumber = JSON_VALUE(@splitdata,'$.L')
		SET @NetValue = JSON_VALUE(@splitdata,'$.V')
		SET @Quantity = JSON_VALUE(@splitdata,'$.Q')
		...
		--DECLARE @LineNumberS NVARCHAR(10),@NetValueS NVARCHAR(10),@QuantityS NVARCHAR(10)
		--SELECT @LineNumber = SUBSTRING(@splitdata, 1,  CHARINDEX('V', @splitdata)-1) ,
		--@NetValue = SUBSTRING(@splitdata, CHARINDEX('V', @splitdata)+1,CHARINDEX('Q', @splitdata) - (CHARINDEX('V', @splitdata)+1)), 
		--@Quantity = SUBSTRING(@splitdata, CHARINDEX('Q', @splitdata)+1, LEN(@splitdata) - (CHARINDEX('Q', @splitdata)))
		...
		INSERT INTO [VirtualStampCard]
		 ...
		 VALUES
			   ...
			   ,@Type)
				...
				END
				ELSE IF ISNULL (@UserId,0) = 0 AND ISNULL (@DeviceIdentifier,0) > 0 AND NOT EXISTS (SELECT 1 FROM PromotionStampCounter  where UserID = 0 AND DeviceIdentifier = @DeviceIdentifier AND  PromotionId = @PromotionId)
				BEGIN
					INSERT INTO [dbo].[PromotionStampCounter] ([Version],[UserId],[PromotionId],[TrxId],[CounterDate],[BeforeValue],[AfterValue],[OnTheFlyQuantity],DeviceIdentifier)
				...
				BEGIN
					...
					ELSE IF ISNULL (@DeviceIdentifier,0) > 0
					BEGIN 
						SELECT @CurrentQuantity = (AfterValue * @ExistingQuantity) + BeforeValue, @AfterValue = (AfterValue * @ExistingQuantity), @BeforeValue = ISNULL(BeforeValue,0)  FROM [PromotionStampCounter]  where DeviceIdentifier = @DeviceIdentifier AND  PromotionId = @PromotionId AND UserID = 0
					END
					...
					IF(ISNULL(@BasketSum,0) + ISNULL(@CurrentQuantity,0)   >= ISNULL(@QualifyingProductQuantity,0))
					BEGIN
						PRINT 'BASKET - StampCard - VALID'
						
						--IF(ISNULL(@VirtualQualifyingProductQuantity,0) + ISNULL(@CurrentQuantity,0)   >= ISNULL(@PromotionThreshold,0))

						SET @StampCount =0
						SET  @StampCount = (ISNULL(@BasketSum,0) + ISNULL(@CurrentQuantity,0)) / @QualifyingProductQuantity
						
						IF ISNULL(@StampCount,0) > 0

						SET @OnTheFlyQuantity = (ISNULL(@BasketSum,0) + ISNULL(@CurrentQuantity,0)) / (ISNULL(@QualifyingProductQuantity,0) + 1); 
						...
						IF ISNULL (@UserId,0) > 0
						BEGIN
							UPDATE [PromotionStampCounter] SET BeforeValue = BeforeValue - @QualifyingProductQuantity * ISNULL(@StampCount,1),TrxId = @TrxId,OnTheFlyQuantity = @OnTheFlyQuantity   where UserId = @UserId AND  PromotionId = @PromotionId
						END
						ELSE IF ISNULL (@DeviceIdentifier,0) > 0
						BEGIN 
							UPDATE [PromotionStampCounter] SET BeforeValue = BeforeValue - @QualifyingProductQuantity * ISNULL(@StampCount,1),TrxId = @TrxId,OnTheFlyQuantity = @OnTheFlyQuantity   where DeviceIdentifier = @DeviceIdentifier AND  PromotionId = @PromotionId AND UserID = 0
						END
...
DEALLOCATE LineItemCursor; 
		...
		BEGIN
			--SELECT * FROM @Result
			SELECT SUM(Result) Result,MAX(RewardId)RewardId,MAX(RewardName)RewardName,
			MAX(VoucherValue)VoucherValue,MAX(DefaultVoucher)DefaultVoucher,MAX(ExistingQuantity)ExistingQuantity,
			MAX(VoucherSubType)VoucherSubType,MAX(StampCardMultiplier)StampCardMultiplier
			FROM @Result	
		END
		ELSE
		BEGIN
	...
	BEGIN
		...
		IF ISNULL(@PromotionId,0) = 236
		BEGIN
			SET @QualifyingProductQuantity = 4
			SET @PromotionCategoryId = 2
			SET @PromotionOfferTypeId = 6
			SET @Reward = ''
			SET @VoucherProfileId = 1050
			SET @StampCardMultiplier = 1
		END
		ELSE IF ISNULL(@PromotionId,0) = 238
		BEGIN
			SET @QualifyingProductQuantity = 6
			SET @PromotionCategoryId = 2
			SET @PromotionOfferTypeId = 6
			SET @Reward = ''
			SET @VoucherProfileId = 1051
			SET @StampCardMultiplier = 1
		END
		ELSE
		BEGIN
			...
			@Reward = ISNULL(Reward,''),
			@VoucherProfileId = ISNULL(VoucherProfileId,0),
		...
		BEGIN
			--SET @RewardName = 
			IF ISNULL(@VoucherProfileId,0) = 1050
			BEGIN
				SET @RewardName = 'Free Tee(s)'
				SET @VoucherValue = 26.99
				SET @VoucherSubType = 'DiscountFixed'
			END
			ELSE IF ISNULL(@VoucherProfileId,0) = 1051
			BEGIN
				SET @RewardName = 'Free Piece(s) of Jewelry'
				SET @VoucherValue = 16.99
				SET @VoucherSubType = 'DiscountFixed'
			END
			ELSE
			BEGIN
