...
-- =============================================
CREATE PROCEDURE [dbo].[EPOS_AddRewardOffer_Optimized_P_BAK18072023](@Trxid int,@ClientId int)
AS
BEGIN
   ...
   -- store all item filters to #itemcode from current trx detail table
	...
	BEGIN
		...
		BEGIN 
			...
			IF exists(SELECT * from promotion where id=CONVERT(int,@INEXPromotionId) )--and PromotionItemFlagAnd = 1)
				BEGIN
					--TOP-666
...
--END BEST OFFER
							...
							BEGIN
								...
								BEGIN
									...
									--BEGIN
										...
										EXEC EPOS_AssignNextAvilableStampCardVoucher @ClientId, @RewardProductId,@OFTBalanceVoucherQty,@MemberId,@Result OUTPUT,@ResultQty OUTPUT,@VoucherProfile OUTPUT,@DeviceIdentifier,@Trxid,@rewardPromoId
									--END
									--ELSE
									--BEGIN
	...
	---------------------END REWARD PROMO TRX------------
			...
			BEGIN 
				...
				@AVTrxDetailId INT,@AVLineNumber INT
							...
							BEGIN
								...
								BEGIN
									...
									BEGIN
										...
										INSERT INTO TrxDetailStampCard VALUES(0,@AVPromotionId,@AVTrxDetailId,ISNULL(@AVAfterValueUpdate,0) *-1,3 )
									END
								END
			...
			END
			 ...
			 BEGIN
				...
				INSERT INTO TrxDetailStampCard 
				SELECT 0 Version,PromotionId,TrxDetailId,vs.Quantity,CASE WHEN vs.Quantity < 0 THEN 2 ELSE 1 END AS PunchTrXType
				FROM #itemcode ic 
				...
				BEGIN 
					...
					--PunchTrXType 3 is burned for reward
					INSERT INTO TrxDetailStampCard 
					SELECT 0 Version,PromotionId,ic.TrxDetailId, (@VoucherQuantity * r.QualifyingProductQuantity )*-1 AS Quantity,3 AS PunchTrXType
					FROM #itemcode ic 
