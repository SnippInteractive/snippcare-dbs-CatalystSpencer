CREATE PROCEDURE [dbo].[MemberSearch]                              
(
		@clientId			INT,
		@memberId			INT,
		...
		@extensionValue NVARCHAR(300) = NULL,
...
BEGIN                              
 ...
 -- interfering with SELECT statements.                              
	...
	(                              
		...
		UserTypeId		INT, 
		UserStatusId	INT,
		AddressStatusTypeid INT,
		AddressTypeid INT,
		MobilePhone nvarchar(20),
		AddressLine2	nvarchar(100) ,
		ExtensionValue nvarchar(100)
	)                              
	...
	Declare @AddressStatusID_Replaced int
	select @AddressStatusID_Replaced = addressstatusid from AddressStatus where name = 'Replaced' and clientid = @clientID              
	...
	Set @FirstRow = @LastRow + 1 - @pageSize                              
		...
		DECLARE @AddressTypeIdMain INT, @AddressTypeIdDelivery INT, 
				@UserTypeIdLM INT,
		...
		SELECT @AddressTypeIdDelivery = AddressTypeId 
		FROM   [AddressType] 
		WHERE  Name = 'Delivery' 
		AND ClientId = @clientId 
		...
		END
		 ...
		 IF   len(@legacyNumber)>0
		 BEGIN
		  select @strWHERE = @strWHERE + ' and u.LegacyNumber = ' + @legacyNumber
		 END                        
...
begin                               
 set @addressLine2 =  replace(@addressLine2 ,'*','%')                              
 set @LikeEqual = ' = '                              
 if CHARINDEX ('%',@addressLine2,0) !=0                              
 begin                               
  set @LikeEqual = ' like '                              
 end                               
 select @strWHERE = @strWHERE + ' and a.AddressLine2 ' + @LikeEqual + ' ''' + @addressLine2 + ''''                              
end                        
...
set @strContactDetails = '  left join usercontactdetails ucd WITH (NOLOCK) on ucd.userid = u.userid left join contactdetails cd on ucd.contactdetailsid = cd.contactdetailsid '  		                      
...
Declare @strDeviceJoins nvarchar(1000)=' ';
 ...
 if isnull(@includeProspect,0) = 0 and  isnull(@onlyProspect,0) = 0  
 begin
 --default search is done only for LoyaltyMembers(if IncludeProspect and onlyProspect checkbox unchecked on search screen)
  select @strWHERE = @strWHERE + '  and u.UserTypeId in ( select UserTypeId from UserType where clientid='+ convert(varchar(5),@clientId) +' and Name in (''EShop'',''LoyaltyMember''))'
 end 	   
...
Declare @extensionName nvarchar(50)='',@strExtensionData nvarchar(500)='',@strextensionColumn nvarchar(100);               
/* User Loyalty Extension Data Search*/
/* If a client want to do a search on a particular extensiondata ,update value for key UserExtensionSearchKey in client config and modify translation for ExtensionValue on member search screen */
SET  @strextensionColumn = ' ''xxx'' ExtensionValue';
Select @extensionName = Value FROM ClientConfig where [KEY] = 'UserExtensionSearchKey' and  ClientId = @clientId
IF len(@extensionName) > 0 
 BEGIN
	set @strWHERE = @strWHERE + '  and ule.PropertyName = ''' + convert(varchar(20),@extensionName) + ''''
	if isnull(@extensionValue,'') !=''                               
	begin                               
	 SET @extensionValue =  replace(@extensionValue ,'*','%')                              
	 SET @LikeEqual = ' = '                              
	 if CHARINDEX ('%',@extensionValue,0) !=0                              
	 begin                               
	  SET @LikeEqual = ' like '                              
	 end                               
	 select @strWHERE = @strWHERE + ' and ule.PropertyValue ' + @LikeEqual + ' ''' + @extensionValue + ''''                              
	end      
	 SET @strExtensionData = ' left join [UserLoyaltyExtensionData] ule  on u.UserLoyaltyDataId=ule.UserLoyaltyDataId '
	 SET @strextensionColumn = ' ule.PropertyValue ExtensionValue'
 END 
   
  /* User Loyalty Extension Data Search - END */  
 -- set @strExtensionData = ' left join [UserLoyaltyExtensionData] ule  on u.UserLoyaltyDataId=ule.UserLoyaltyDataId '                          
DECLARE @strQueryMain NVARCHAR(MAX) = ''  
                           
SET @strQueryMain = 'INSERT INTO #userData(UserId, SiteName, SurName,FirstName,StreetHnr,City,Zip,MemberStatus, UserTypeId, UserStatusId,ActivityStatus,AddressLine1,AddressStatusTypeID, AddressTypeID,MobilePhone,AddressLine2,ExtensionValue)  SELECT  DISTINCT  u.UserId ,          
 s.name as SiteName,             
 ...
 + 'us.name ActivityStatus, isnull(a.AddressLine1,'''')  AddressLine1,AddressStatusID,AddressTypeID,cd.MobilePhone,isnull(a.AddressLine2,'''')  AddressLine2,'+ @strextensionColumn +'  FROM   [User] u WITH (NOLOCK)                    
 inner join [PersonalDetails] pd WITH (NOLOCK) on u.PersonalDetailsId = pd.PersonalDetailsId                             
 inner join [Site] s WITH (NOLOCK) on s.siteid = u.siteid   and s.ClientId =  '+ convert(varchar(5),@clientId) +'                 
 left join  [UserAddresses] ua  WITH (NOLOCK) on ua.userid = u.userid                             
 left join Address a  WITH (NOLOCK) on ua.AddressId = a.AddressId                               
  '+ @strDeviceJoins+' ' +  @strprofileExtraInfo + '  '+ @strExtensionData +'  '+ @strContactDetails  +'                       
...
delete from #userData where AddressStatusTypeid = @AddressStatusID_Replaced
delete from #userData where AddressTypeid = @AddressTypeIdDelivery                              
print @strQuery                              
...
set @strQuery = 'INSERT INTO #userDataDistinct (UserId,SiteName,SurName, FirstName , StreetHnr,City,Zip,MemberStatus,ActivityStatus,AddressLine1,MobilePhone,AddressLine2,ExtensionValue,TotalCount,RowNumber)  Select UserId,SiteName,SurName,FirstName,StreetHnr,City,Zip,                      
  
MemberStatus,ActivityStatus,AddressLine1,MobilePhone,AddressLine2,ExtensionValue,TotalCount ,ROW_NUMBER() OVER(' + @sqlOrder + ' ) as RowNumber  from ( ' +                              
'select distinct top '+convert(varchar(5),@MaxResult)+' UserId,SiteName,SurName,FirstName,StreetHnr,City,Zip,  MemberStatus,ActivityStatus,AddressLine1,MobilePhone,AddressLine2,ExtensionValue,0 TotalCount from [#userData]  ) x'                   
print @strQuery                              
exec (@strQuery)                           
...
--begin                              
 ...
 set @strQuery = 'select UserId,SiteName,SurName,FirstName,StreetHnr,City,Zip,  MemberStatus,ActivityStatus, AddressLine1,MobilePhone,AddressLine2,ExtensionValue, dbo.GetActiveLoyaltyDeviceId(UserId) as ''ActiveLoyaltyDeviceId'', ' + convert(varchar(3),@RecCount) + ' as TotalCount '+@nullReturnAddition+' from [#userDataDistinct] where rownumber  between  '   
