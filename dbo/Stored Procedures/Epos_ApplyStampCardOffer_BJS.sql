...
CREATE PROCEDURE [dbo].[Epos_ApplyStampCardOffer_BJS]
	...
	(@TrxId INT,@DeviceId NVARCHAR(25),@SeviceCall INT=0)
AS
BEGIN
...
BEGIN
	...
	RewardName NVARCHAR(250),ValidLineNumbers NVARCHAR(2500),Reward NVARCHAR(500),RewardId INT,AnonymizeAfterValue DECIMAL(18,2),DefaultVoucherCount INT,TrxDetailStampsBeforeValue float)
	...
	CREATE TABLE #TrxVoucherdetail(TrxDetailId INT ,TrxVoucherId NVARCHAR(30),PromotionId INT,TrxVoucherDetailId INT)
	...
	CREATE TABLE #updateVirtualStampCard (Id INT Identity (1,1),TrxVoucherId NVARCHAR(25),Linenumber INT,VoucherProfileId INT,PromotionId INT,QualifyingProductQuantity float,
	...
	UPDATE vs SET vs.TrxDetailId = td.TrxDetailId
	...
	QualifyingProductQuantity,VoucherProfileId,StampCardMultiplier,Reward)--,QualifyiedTrxDetailId,PromotionOfferType,PromotionCategory,AnonymizeAfterValue
	SELECT DISTINCT PSC.Id,PSC.Version,PSC.UserId,PSC.PromotionId,PSC.TrxId,CounterDate,BeforeValue,AfterValue,PreviousStampCount,OnTheFlyQuantity,DeviceIdentifier,P.Name,
	P.QualifyingProductQuantity,P.VoucherProfileId,ISNULL(P.StampCardMultiplier,1),P.Reward
	...
	UPDATE #VirtualStampCard SET ActualQuantity = FLOOR((ISNULL(Quantity,0) - ISNULL(ChildPunch,0))/ISNULL(StampCardMultiplier,1)) WHERE StampCardType = 'StampCardQuantity' AND Quantity > 0
	UPDATE #VirtualStampCard SET SingleNetValue = NetValue / ISNULL(ActualQuantity,1) --WHERE StampCardType = 'StampCardValue'  AND Quantity > 0
	...
	from #TrxVoucherdetail tv 
	inner join #TrxDetail td on tv.trxdetailid  = td.trxdetailid 
	inner join Device d on tv.TrxVoucherId  collate database_default = d.DeviceId 
	...
	BEGIN 
			...
			BEGIN
				UPDATE #VirtualStampCard SET Stamps = ISNULL(Stamps,0) - (@vsStampCardMultiplier * CASE StampCardType WHEN 'StampCardQuantity' THEN 1 ELSE SingleNetValue END) 
			...
			ELSE
			BEGIN
				UPDATE #VirtualStampCard SET Stamps = ISNULL(Stamps,0) - (@vsStampCardMultiplier * CASE StampCardType WHEN 'StampCardQuantity' THEN 1 ELSE SingleNetValue END) --(@vsStampCardMultiplier * (@vsQualifyingProductQuantity+1)) 
				Where trxid = @TrxId AND VoucherId = @vsVoucherProfileId AND PromotionId = @vsPromotionId AND Linenumber = @vsLinenumber  AND PromotionOfferType  = 'Voucher' AND Quantity > 0 --AND UsageType = 'UNIQUE'
			END
	...
	--SELECT * FROM #PromotionStampCounter
	----REWARD PROMO - NOT USING SPENCER
 ...
 --   SET td.PromotionId = (
	...
	--SELECT * FROM #VirtualStampCard
	insert into TrxDetailStampCard(PromotionId,TrxDetailId,ValueUsed,PunchTrXType,ChildPromotionId,ChildPunch)
	select DISTINCT PromotionId,TrxDetailId,isnull(Stamps,0) ValueUsed,1 AS PunchTrXType,ChildPromotionId,ChildPunch
	from #VirtualStampCard Where ISNULL(TrxDetailId,0) > 0  And Quantity <> 0 AND isnull(Stamps,0) >=0
	UNION 
	select DISTINCT PromotionId,TrxDetailId,isnull(Stamps,0) ValueUsed,2 AS PunchTrXType,ChildPromotionId,ChildPunch
	from #VirtualStampCard Where ISNULL(TrxDetailId,0) > 0  And Quantity <> 0 AND isnull(Stamps,0) <0
	UNION 
	SELECT PromotionId,QualifyiedTrxDetailId, TrxDetailStampsBeforeValue,3 AS PunchTrXType,null,null 
	FROM #PromotionStampCounter WHERE StampReward > 0 AND ISNULL(QualifyiedTrxDetailId,0) > 0
