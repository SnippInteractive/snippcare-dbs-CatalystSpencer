...
-- Modified Date: 2021-10-06,14-08-2023
-- =============================================
CREATE PROCEDURE [dbo].[EPOS_AddRewardOffer_Optimized](@Trxid int,@ClientId int)
AS
BEGIN
	...
	DECLARE @CriteriaIdQuantity INT,@CriteriaIdValue INT,@PromotionCategoryRebateId INT,@TrxtypeIdReward INT,@ApplicationNumber nvarchar(50),@ReceiptId int,@DeviceIdentifier INT,@AccountId INT,@IsStampClaim BIT = 0;
	...
	DECLARE @PromotionOfferTypeReward INT,@PromotionCategoryTypeTransactionBased INT
	SELECT @PromotionOfferTypeReward = Id from PromotionOfferType WHERE Name  = 'Reward' AND ClientId = @ClientId
	...
	SET @DeviceStatusIdInactive=(select DeviceStatusId from DeviceStatus With(nolock) where Name='Inactive' and ClientId=@ClientId)
	SET @ProfileStatusIdInactive=(select DeviceProfileStatusId from DeviceProfileStatus With(nolock) where Name='Inactive' and ClientId=@ClientId)
   ...
   -- store all item filters to #itemcode from current trx detail table
	...
	DECLARE @AllEnabledRewardOffer TABLE (PromotionId VARCHAR(50),Name VARCHAR(250),LineNumber INT,TrxDetailID INT,Points float,BonusPoints INT,OfferType VARCHAR(50)
	...
	--Fetch All Reward LineItem And basket Promotions
		--INCLUDE
		SELECT DISTINCT PromotionId INTO #EXVirtualPointPromotions from  VirtualPointPromotions where trxid=@Trxid and PromotionId>0
		--tmp.LineNumber, tmp.TrxDetailID, tmp.Points,pt.Code,pt.Quantity as itemQuantity, tmp.Quantity as trxQuantity,tmp.value NetValue,pt.ItemIncludeExclude,
		INSERT INTO @AllEnabledRewardOffer
		SELECT  distinct  CONVERT(varchar(50), p.Id ) as PromotionId, p.Name, 0 AS LineNumber, 0 AS TrxDetailID, 0 AS Points,0 as BonusPoints ,'Reward' as OfferType,
		'Reward' as Offer,'' AS Code,p.CriteriaId,0 AS  itemQuantity, 0 AS  trxQuantity,Cumulation,
		...
		,0 AS NetValue,p.QualifyingProductQuantity,	CASE P.PromotionTypeId when 1 then 'Basket' else 'LineItem' end AS PromotionType,
		'' AS StampCardType,0 AS RewardQuantity,'' AS RewardLines,MaxPromoApplicationOnSameBasket,'' AS ItemIncludeExclude,
		PromotionThreshold,p.PromotionCategoryId,p.PromotionUsageLimit,p.MaxUsagePerMember,0 AS MaxBasketLimit,'SKU-NA' AS ItemCode,GroupItems,	0 AS OnTheFlyQuantity,
		0 AS OnTheFlyUsedQuantity,p.MisCode,p.StampCardMultiplier
		...
		where p.StartDate<=@Trxdate and p.ENDDate>=@Trxdate and p.IsTemplate=0 
		and p.Enabled=1 and P.PromotionOfferTypeId = @PromotionOfferTypeReward 
		...
		and @SiteId in (SELECT SiteId from GetChildSitesBySiteId(ps.siteid))
		and p.Id not in(SELECT PromotionId from #EXVirtualPointPromotions) 
		PRINT @SiteId
		PRINT @PromotionOfferTypeReward
		PRINT @PromotionCategoryTypeTransactionBased
		--SELECT * FROM @AllEnabledRewardOffer

		INSERT INTO @RewardOffer
		SELECT  distinct  P.PromotionId, p.Name, tmp.LineNumber, tmp.TrxDetailID, tmp.Points,BonusPoints ,OfferType,Offer, pt.Code,p.CriteriaId, 
		pt.Quantity as itemQuantity, tmp.Quantity as trxQuantity, Cumulation,RewardId,RewardName,ProductId,RewardValue
		,tmp.value NetValue,p.QualifyingProductQuantity,PromotionType,StampCardType,RewardQuantity,RewardLines,MaxPromoApplicationOnSameBasket ,
		pt.ItemIncludeExclude,PromotionThreshold,p.PromotionCategoryId,p.PromotionUsageLimit,p.MaxUsagePerMember,MaxBasketLimit,ItemCode,GroupItems,
		OnTheFlyQuantity,OnTheFlyUsedQuantity ,p.MisCode,p.StampCardMultiplier
		from @AllEnabledRewardOffer p 
		inner join PromotionItem pt with(nolock)  on p.PromotionId=pt.PromotionId
		inner join #tempCode tmp with(nolock) on  pt.PromotionItemTypeId=tmp.TypeId
		where RTRIM(LTRIM(pt.Code))= RTRIM(LTRIM(tmp.Code))
		and pt.ItemIncludeExclude = 'IncludeItem'

		--ALL INCLUDE
		select distinct P.PromotionId into #AllIncludeOffer
		FROM @AllEnabledRewardOffer P  
		INNER JOIN PromotionItem pit with(nolock) ON P.PromotionId = pit.PromotionId
		WHERE pit.ItemIncludeExclude = 'IncludeItem'
		...
		inner join PromotionItem pt with(nolock)  on p.PromotionId=pt.PromotionId
		inner join #tempCode tmp with(nolock) on  pt.PromotionItemTypeId=tmp.TypeId
		where pt.ItemIncludeExclude = 'ExcludeItem'
		AND pt.Id NOT IN (SELECT DISTINCT PromotionId FROM #AllIncludeOffer)
			...
			IF exists(SELECT 1 from promotion where id=CONVERT(int,@INEXPromotionId) )--and PromotionItemFlagAnd = 1)
				BEGIN
					...
					BEGIN
						...
						IF exists(SELECT 1 from PromotionSegments p with(nolock) join segmentadmin s with(nolock) on p.segmentid=s.segmentid where validto >= GETDATE() and PromotionId=CONVERT(int, @INEXPromotionId) )
							BEGIN
							--Check again if any segment filter exists and belong to that user
							IF not exists(SELECT 1 from PromotionSegments ps with(nolock) join SegmentUsers s with(nolock) on ps.SegmentId = s.SegmentId where ps.PromotionId = CONVERT(int, @INEXPromotionId) and s.UserId = @MemberId)
								BEGIN
				...
				SELECT @MaxUsagePerMemberLimit = isnull(MaxUsagePerMember,0), @INEXPromotionUsageLimit= isnull(PromotionUsageLimit,0) from Promotion with(nolock) where Id =@INEXPromotionId
	...
	IF EXISTS (Select 1 from @RewardOffer)
		BEGIN
				...
				BEGIN
					...
					--SET @UsedCountR=(SELECT count(promotionid) from [PromotionRedemptionCount] WITH(NOLOCK) where [MemberId] = @MemberId and  PromotionId = @rewardPromoIdR AND ItemCode = @rewardItemCodeR)
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								INNER JOIN PromotionStampCounter ps with(nolock) on vt.PromotionId = ps.PromotionId and ps.UserId = @MemberId
							END
							ELSE IF ISNULL (@DeviceIdentifier,0) > 0
							BEGIN 
								INSERT INTO @OTFStampVoucherOffer
								SELECT vt.RewardQuantity,vt.QualifyingProductQuantity,vt.PromotionId,ps.OnTheFlyQuantity FROM #OTFVoucherOffer vt 
								INNER JOIN PromotionStampCounter ps with(nolock) on vt.PromotionId = ps.PromotionId and ISNULL(ps.UserId,0) = 0 AND ISNULL(ps.DeviceIdentifier,0) = @DeviceIdentifier
							END
			...
			select @trxTypeReward= TrxTypeId from TrxType WITH(NOLOCK) where Name='Reward' and ClientId=@ClientId;
			SELECT @trxstatusStarted = TrxStatusId FROM TrxStatus WITH(NOLOCK) WHERE [Name]='Started' AND Clientid = @ClientId;
			...
			--and link the new reward trx with original trx by adding trxid to OLD_TrxId field
				...
				BEGIN
					...
					FROM TrxDetail td WITH(NOLOCK) INNER JOIN TrxHeader th WITH(NOLOCK) ON td.TrxID = th.TrxId 
				...
				BEGIN				 
					--SET @UsedCount=(SELECT count(promotionid) from [PromotionRedemptionCount] WITH(NOLOCK) where [MemberId] = @MemberId and  PromotionId = @rewardPromoId AND ItemCode = @ItemCode)
				...
				BEGIN				 
					...
					--SET @maxTrxCountforPromtion=(SELECT count(promotionid) from [PromotionRedemptionCount] WITH(NOLOCK) where PromotionId = @rewardPromoId)
						...
						BEGIN
							...
							BEGIN
								...
								BEGIN
									...
									--PRINT 'On The Fly Voucher Redemtion'
									--PRINT @rewardQuantity
									--PRINT'***************************'
									...
									--BEGIN
										...
										--IF ISNULL (@MemberId,0) > 0
										--BEGIN
										--	SELECT TOP 1 @OFTUnUsedtrxQty = AfterValue + ABS(BeforeValue) FROM [PromotionStampCounter] With(NOLOCK) WHERE PromotionId = @rewardPromoId and UserId = @MemberId
										--END
										--ELSE IF ISNULL (@DeviceIdentifier,0) > 0
										--BEGIN 
										--	SELECT TOP 1 @OFTUnUsedtrxQty = AfterValue + ABS(BeforeValue) FROM [PromotionStampCounter] With(NOLOCK) WHERE PromotionId = @rewardPromoId and isnull(UserId,0) = 0 AND ISNULL(DeviceIdentifier,0) = @DeviceIdentifier
										--END
									...
									BEGIN
										...
										CREATE TABLE  #VoucherList (Id INT identity (1,1), VoucherId nvarchar(50))
										--SELECT * INTO #VoucherList FROm SplitString(@Result,',')
										DECLARE @InsertStatement varchar(max);
										SET  @InsertStatement = 'insert into #VoucherList (VoucherId) values ('''+REPLACE(@Result,',','''),(''')+''');';
										--select @InsertStatement
										EXEC (@InsertStatement);
										--select * from #VoucherList

										DROP TABLE IF EXISTS #UpdateVoucherdetail
										select ROW_NUMBER() OVER(ORDER BY tv.Id ASC) AS ItemIndex, tv.* INTO #UpdateVoucherdetail
										from trxvoucherdetail tv with(nolock) inner join trxdetail td with(nolock) on tv.trxdetailid = td.trxdetailid 
										where  trxid = @TrxId AND TrxVoucherId = @DefaultVoucher + convert(nvarchar(25),@rewardPromoId)

										DECLARE @OTFVoucherId NVARCHAR(25) = '',@OTFtrxVoucherDetailId INT,@DefaultVoucherCount INT;

										IF EXISTS (SELECT 1 FROM #UpdateVoucherdetail)
										BEGIN
											--SELECT * FROM #UpdateVoucherdetail
											--SELECT * FROM #VoucherList
											SELECT @DefaultVoucherCount = Count(TrxDetailId) FROM #UpdateVoucherdetail
											DECLARE @UpdateCounter INT= ISNULL(@DefaultVoucherCount,0),@VoucherDeviceId INT;
											WHILE (@UpdateCounter > 0)
											BEGIN
												
												SELECT @OTFVoucherId = VoucherId FROM #VoucherList WHERE Id = @UpdateCounter
	...
	---------------------END REWARD PROMO TRX------------
							...
							BEGIN
								...
								BEGIN
									...
									BEGIN
										...
										INSERT INTO TrxDetailStampCard (Version,PromotionId,TrxDetailId,ValueUsed,PunchTrXType,ChildPromotionId,ChildPunch) VALUES(1,@AVPromotionId,@AVTrxDetailId,ISNULL(@AVAfterValueUpdate,0) *-1,3,@ChildPromotionId ,@ChildPunch)
									END
								END
			...
			END
			 ...
			 BEGIN
				...
				DECLARE @VoucherLinenumbers NVARCHAR(MAX),@VoucherPromotionId INT,@VoucherQuantity INT,@VoucherLineNumber INT
				...
				BEGIN 
					...
					--SET @VoucherLineNumber = left(@VoucherLinenumbers, charindex(',', @VoucherLinenumbers) - 1)
					
					if charindex(',', @VoucherLinenumbers) !=0
					BEGIN
						SET @VoucherLineNumber = left(@VoucherLinenumbers, charindex(',', @VoucherLinenumbers) - 1)
					END
					ELSE
					BEGIN
						SET @VoucherLineNumber = @VoucherLinenumbers
					END
					PRINT @VoucherLineNumber

					--PunchTrXType 3 is burned for reward
					INSERT INTO TrxDetailStampCard (Version,PromotionId,TrxDetailId,ValueUsed,PunchTrXType,ChildPromotionId,ChildPunch)
					SELECT DISTINCT 0 Version,PromotionId,ic.TrxDetailId, (@VoucherQuantity * r.QualifyingProductQuantity )*-1 AS ValueUsed,3 AS PunchTrXType,0 AS ChildPromotionId,0 AS ChildPunch
					FROM #itemcode ic 
					...
					WHERE ic.Linenumber =  @VoucherLineNumber AND r.StampCardType IN('StampCardQuantity','StampCardValue') AND r.Offer IN('Reward','Voucher')
				FETCH NEXT FROM OnlineCursor     
				INTO @VoucherLinenumbers,@VoucherPromotionId     ,@VoucherQuantity  
				END     
				CLOSE OnlineCursor;    
