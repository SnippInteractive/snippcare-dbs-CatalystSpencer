...
--END TOP 666
	...
	where vt.voucherprofileitemtypeid=tmp.TypeId and  RTRIM(LTRIM(vt.Code))= RTRIM(LTRIM(tmp.Code)) and d.StartDate<=@TrxDate and d.EXpirationdate>=@TrxDate  and d.devicestatusid=@DeviceStatusActive and 
	...
	BEGIN  
		...
		device d with(nolock) join deviceprofile dp with(nolock) on d.id=dp.deviceid 
		join deviceprofiletemplate dpt with(nolock) on dp.deviceprofileid=dpt.id 
		join deviceprofiletemplatetype dpty with(nolock) on dpt.deviceprofiletemplatetypeId=dpty.id
		join voucherdeviceprofiletemplate vdp with(nolock) on dpt.id=vdp.id where dpty.name='Voucher' and d.DeviceId=@VoucherId and vdp.ClassicalVoucher=1
