...
-- =============================================
CREATE PROCEDURE [dbo].[Epos_StampCardCalculation_WIP]
...
BEGIN
	...
	*/
	...
	BEGIN
		...
		FROM PromotionStampCounter with(nolock) WHERE UserId =  @UserId AND PromotionId = @PromotionId 
	END
	ELSE IF ISNULL (@DeviceIdentifier,0) > 0
	BEGIN 
		SELECT  @PromotionStampCounterId = Id,@AfterValue = ISNULL(AfterValue,0),@BeforeValue = ISNULL(BeforeValue,0), @OnTheFlyQuantity = ISNULL(OnTheFlyQuantity,0)
		FROM PromotionStampCounter with(nolock) WHERE DeviceIdentifier =  @DeviceIdentifier AND PromotionId = @PromotionId
	END
...
BEGIN
	...
	--SET @DefaultVoucher = ''
	UPDATE li SET Stamps = (Quantity * -1) * ISNULL((SELECT TOP 1 StampReturn FROM #ReturnStampCardItemDetails WHERE ItemCode = li.ItemCode Order BY TrxDetailId DESC),ABS(ISNULL(NetValue,0))/Quantity) 
	FROM #LineItem li WHERE NetValue  < 0

	SELECT  (Quantity * -1) * ISNULL((SELECT TOP 1 StampReturn FROM #ReturnStampCardItemDetails WHERE ItemCode = li.ItemCode Order BY TrxDetailId DESC),ABS(ISNULL(NetValue,0))/Quantity) 
	FROM #LineItem li WHERE NetValue  < 0
