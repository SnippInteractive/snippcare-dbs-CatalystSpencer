...
CREATE VIEW [dbo].[Transactions_Header]  
...
SELECT DISTINCT   
						 ...
						 tdstamp.PromotionID,TerminalDescription,						
						 CASE WHEN tdstamp.PunchTrXType in (1,2) THEN promo.[Name] ELSE NULL END as PromotionName,
						 ISNULL(stm.Stamps,0) as StampCount, -- lineitemlvl
						 ...
						 ISNULL(stm.Stamps,0)as Stamps  --headerlvl
...
FROM            dbo.TrxHeader AS TH LEFT OUTER JOIN  
						 ...
						 (SELECT         SUM(tdstamp.ValueUsed) as Stamps,td.TrxId
                               FROM            dbo.TrxDetail td left JOIN                                          
											   dbo.TrxDetailStampCard tdstamp on tdstamp.TrxDetailId = td.TrxDetailId and tdstamp.PunchTrXType in (1,2)								  
                              GROUP BY td.TrxId) AS stm on stm.TrxID =th.TrxId inner join  
                             (SELECT        SUM(dbo.TrxDetail.Value) AS TotalValue, 0 AS TotalPoints, 0 AS TotalBonus,   
                                                         SUM(dbo.TrxDetail.EposDiscount + dbo.TrxDetail.LoyaltyDiscount) AS TotalDiscount, SUM(dbo.TrxDetail.Value) AS Amount,
														  dbo.TrxHeader.TrxId ,0 as TotalPromoValue, 0 as Stamps
