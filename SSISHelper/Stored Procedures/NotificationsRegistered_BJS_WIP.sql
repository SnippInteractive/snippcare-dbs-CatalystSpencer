...
CREATE Procedure [SSISHelper].[NotificationsRegistered_BJS_WIP] (
...
--BEGIN TRAN
	...
	Begin
		...
		--SPEN-T33
		update NotificationsToSend set placeholders = (SELECT Content FROM OpenJson(placeholders)WITH (Content NVARCHAR(150) '$.Content')) 	
		where SentDate is null and NotificationType='SMS' AND ISJSON(placeholders) = 1 

		update NotificationsToSend set placeholders = (SELECT Body FROM OpenJson(placeholders)WITH (Body NVARCHAR(150) '$.Body')) 	
